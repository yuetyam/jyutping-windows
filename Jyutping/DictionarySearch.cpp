...
TryAgain:
    ...
    else
    {
        ...
        *ppdret = new (std::nothrow) CDictionaryResult();
        ...
        CJyutpingArray<CParserStringRange> commentStrings;
        if (!ParseLine(&pwch[indexTrace], bufLenOneLine, &keyword, &valueStrings, &commentStrings))
        {
            if (*ppdret)
            {
                delete *ppdret;
        ...
        for (UINT i = 0; i < valueStrings.Count(); i++)
        {
            ...
            if (i < commentStrings.Count())
            {
                 CStringRange* findPhraseComment = (*ppdret)->_FindPhraseCommentList.Append();
                 if (findPhraseComment)
                 {
                     *findPhraseComment = *commentStrings.GetAt(i);
                 }
            }
            else
            {
                // Ensure the list size matches if comment is missing for some reason (though strictly it shouldn't be)
                 CStringRange* findPhraseComment = (*ppdret)->_FindPhraseCommentList.Append();
                 // Empty comment
            }
        }
