...
template<class T>
class CJyutpingArray
{
    typedef typename std::vector<T> CJyutpingInnerArray;
    typedef typename std::vector<T>::iterator CJyutpingInnerIter;

public:
    CJyutpingArray(): _innerVect()
    {
    }

    explicit CJyutpingArray(size_t count): _innerVect(count)
    {
    }

    virtual ~CJyutpingArray()
    {
    }
    ...
    void RemoveAt(size_t index)
    {
        ...
        CJyutpingInnerIter iter = _innerVect.begin();
...
private:
    CJyutpingInnerArray _innerVect;
};
